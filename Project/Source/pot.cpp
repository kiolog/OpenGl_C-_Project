...
class Pot 
{
	...
	mat4 pot_model = rotate(mat4(), radians(GLfloat(-90.0)), vec3(1.0, 0.0, 0.0)) * scale(mat4(), vec3(1, 1, 1 ));
	int open;
	int t1;
...
public:
	void InitPotprogram()
	{
		pot_program.program = create_program("pot_vertex.glsl", "pot_fragment.glsl");
		pot_program.um4p = glGetUniformLocation(pot_program.program, "um4p");
		pot_program.um4m = glGetUniformLocation(pot_program.program, "um4m");
		pot_program.um4v = glGetUniformLocation(pot_program.program, "um4v");
		pot_program.tex = glGetUniformLocation(pot_program.program, "tex");
		pot_program.snow_open = glGetUniformLocation(pot_program.program, "snow_open");
		loadModel("Poinsetta.obj", "Poinsetta.mtl");
	}
	void DrawPot(mat4 _ViewMatrix, mat4 _ProjMatrix) {
		glUseProgram(pot_program.program);

		glUniformMatrix4fv(pot_program.um4m, 1, GL_FALSE, value_ptr(pot_model));
		glUniformMatrix4fv(pot_program.um4v, 1, GL_FALSE, value_ptr(_ViewMatrix));
		glUniformMatrix4fv(pot_program.um4p, 1, GL_FALSE, value_ptr(_ProjMatrix));

		glActiveTexture(GL_TEXTURE0);
		glUniform1i(pot_program.tex, 0);
		if(open)
			glUniform1i(pot_program.snow_open, 1);
		else 
			glUniform1i(pot_program.snow_open, 0);
		//draw
		for (int i = 0; i < t1; ++i)
		{
			...
			glDrawElementsInstanced(GL_TRIANGLES, shapes[i].drawCount, GL_UNSIGNED_INT, 0, 10);
		}
	}
	void DrawPot(Camera myCamera)
	{
		glUseProgram(pot_program.program);

		glUniformMatrix4fv(pot_program.um4m, 1, GL_FALSE, value_ptr(pot_model));
		glUniformMatrix4fv(pot_program.um4v, 1, GL_FALSE, value_ptr(myCamera.getViewMatrix()));
		glUniformMatrix4fv(pot_program.um4p, 1, GL_FALSE, value_ptr(myCamera.getProjectionMatrix()));
