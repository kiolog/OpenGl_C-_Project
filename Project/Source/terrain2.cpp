...
public:
	...
	void loadUniforms(Camera& _camera, mat4 modelMatrix){
		...
		glUniform3fv(uniforms.lightPosition, 1, value_ptr(_camera.getLightPose()));
	};
	void loadUniforms(mat4 &_ViewMatrix,mat4 &_ProjMatrix, mat4 &modelMatrix,vec4 &_v4Plane,vec3 &_v3LightPosition,mat4 &_ShadowMatrix,GLuint _iShadowMap) {
	...
	struct {
		...
		void freeMemory(void) {
			if (ini) {
				...
				// freeFloatPointer(mountain);
				// freeFloatPointer(river);
				// freeFloatPointer(hill);
           	...
           	for(float j = 0; j < VERTEX_COUNT ; j++){
				...
				float rawVal = heightData.raw[coord];
     			mtVal *= (0.5 + 0.5 * rawVal) * format.mountainHeight;
           		riVal *= (0.2 + 0.8 * rawVal) * (format.riverDepth - 5);
           		hiVal *= (0.6 + 0.4 * rawVal) * format.hillHeight;
				rawVal *= 0.01 * format.plainHeight + 5;
...
class TerrainGenerator{
    ...
    void intialHeightGenerator(void){
		...
		heightGenerator.setHeightFormat(mtH,2,0.3*mtH ,-0.1*mtH);
...
public:
	...
	bool validPosition(vec3 pos){
		if (pos.y <= generator.getHeight(pos.x,pos.z))
			return false;
		float distToOrigin = glm::length(vec2(pos.x, pos.z));
	...
	void render(Camera& _camera, TerrainProgram& _program){
		_program.start();
	...
	void render(mat4 &_ViewMatrix,mat4 &_ProjMatrix,vec3 &_v3LightPosition,vec4 &_v4Plane,mat4 &_ShadowMatrix,GLuint _iShadowMap, TerrainProgram& _program) {
		//PrintMat4("ViewMatrix", _ViewMatrix);
		//PrintMat4("ProjMatrix", _ProjMatrix);
		_program.start();
			_program.loadUniforms(_ViewMatrix, _ProjMatrix,modelMatrix, _v4Plane, _v3LightPosition, _ShadowMatrix, _iShadowMap);
			glBindVertexArray(vao);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexVbo);
			glDrawElements(GL_TRIANGLES, (VERTEX_COUNT - 1)*(VERTEX_COUNT - 1) * 6, GL_UNSIGNED_INT, (void*)0);
			glBindVertexArray(0);
